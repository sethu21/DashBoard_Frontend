...
export default async function handler(req, res) {
  try {
    // Query water content data from Port 2, sorted by timestamp
    const query = `
      SELECT 
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'No ET data found for Port 2.' });
    }
    ...
    console.error("Database query error (Port 2):", error);
